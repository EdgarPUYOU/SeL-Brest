...
var filter = (function () {
	...
	return function(inputs, minValues, maxValues, multiplexIndex){
		var result = [];
		for(var i = 0; i < inputs.length; i++)
		{
			if (inputs[i] > lastValue) {
				result[i] = 2;
				script.log(result[i]);
			} else if (inputs[i] < lastValue) {
				result[i] = 1;
				script.log(result[i]);
			} else if (inputs[i] == lastValue && lastValue == 127) {
				result[i] = 3;
				script.log(result[i]);
			} else if (inputs[i] == lastValue && lastValue == 0) {
				result[i] = 4;
				script.log(result[i]);
			}
			lastValue = inputs[i];
